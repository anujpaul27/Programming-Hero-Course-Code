...
export default async function Page({ params }) {
  ...
  if (user.role !== 'seeker')
  {
    return (
      <div>
        <p className="text-red-400 text-center mt-4">Just Seecker Can Apply to the job</p>
      </div>
    )
  }
  ...
  // count length 
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/job/apply/count/${user.id}`)
